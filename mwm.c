...
typedef struct Client {
    struct Client *next;
    Bool isurgn, isfull/*, isfloat*/, istrans, immutable;
    Window win;
    int x, y, w, h;
} Client;
...
static void focus(Client *, Desktop *);
...
static void removeclient(Client *, Desktop *);
static void run(void);
static void setfullscreen(Client *, Desktop *, Bool);
static void setup(void);
static void sigchld(int);
static void stack(int, int, int, int, const Desktop *);
static void arrange(Desktop *);
...
void change_desktop(const Arg *arg) {
    ...
    if (n->curr) XMapWindow(dpy, n->curr->win);
    ...
    for (Client *c = d->head; c; c = c->next) if (c != d->curr) XUnmapWindow(dpy, c->win);
    if (d->curr) XUnmapWindow(dpy, d->curr->win);
    XChangeWindowAttributes(dpy, root, CWEventMask, &(XSetWindowAttributes){.event_mask = ROOTMASK});
    if (n->head) { arrange(n); focus(n->curr, n); }
    desktopinfo();
}
...
void client_to_desktop(const Arg *arg) {
    ...
    c->next = NULL;
    XChangeWindowAttributes(dpy, root, CWEventMask, &(XSetWindowAttributes){.do_not_propagate_mask = SubstructureNotifyMask});
    if (XUnmapWindow(dpy, c->win)) focus(d->prev, d);
    XChangeWindowAttributes(dpy, root, CWEventMask, &(XSetWindowAttributes){.event_mask = ROOTMASK});
    if (!(c->isfloat || c->istrans) || (d->head && !d->head->next)) arrange(d);
 ...
 * to disallow this behavior, we 'arrange()' the desktop to which
...
void configurerequest(XEvent *e) {
    ...
    Desktop *d = NULL; Client *c = NULL;
    if (wintoclient(ev->window, &c, &d)) arrange(d);
}
...
void enternotify(XEvent *e) {
    ...
    focus(c, d);
    if (p) XChangeWindowAttributes(dpy, p->win, CWEventMask, &(XSetWindowAttributes){.event_mask = EnterWindowMask});
}
...
void grabbuttons(Client *c) {
    ...
    for (m = 0; CLICK_TO_FOCUS && m < LENGTH(modifiers); m++)
        if (c != desktops[currdeskidx].curr) XGrabButton(dpy, FOCUS_BUTTON, modifiers[m],
...
void killclient(void) {
    ...
    if (n < 0) { XKillClient(dpy, d->curr->win); removeclient(d->curr, d); }
...
void maprequest(XEvent *e) {
    ...
    if (wintoclient(w, &c, &d) || (XGetWindowAttributes(dpy, w, &wa) && wa.override_redirect))
        return;
...
void mousemotion(const Arg *arg) {
    ...
    if (!d->curr || !XGetWindowAttributes(dpy, d->curr->win, &wa)) return;

    if (arg->i == RESIZE) XWarpPointer(dpy, d->curr->win, d->curr->win, 0, 0, 0, 0, --wa.width, --wa.height);
    int rx, ry, c, xw, yh; unsigned int v; Window w;
    if (!XQueryPointer(dpy, root, &w, &w, &rx, &ry, &c, &c, &v) || w != d->curr->win) return;
    ...
    if (!d->curr->isfloat && !d->curr->istrans) { d->curr->isfloat = True; arrange(d); focus(d->curr, d); }
...
void move_down(void) {
    ...
    if (!d->curr->isfloat && !d->curr->istrans) arrange(d);
}
...
void setfullscreen(Client *c, Desktop *d, Bool fullscrn) {
    if (fullscrn != c->isfull) XChangeProperty(dpy, c->win,
            netatoms[NET_WM_STATE], XA_ATOM, 32, PropModeReplace, (unsigned char*)
            ((c->isfull = fullscrn) ? &netatoms[NET_FULLSCREEN]:0), fullscrn);
    if (fullscrn) XMoveResizeWindow(dpy, c->win, 0, 0, ww, wh);
    XSetWindowBorderWidth(dpy, c->win, (c->isfull || !d->head->next ? 0:BORDER_WIDTH));
}
...
void setup(void) {
    ...
    ww = XDisplayWidth(dpy,  screen);
    wh = XDisplayHeight(dpy, screen);
    ...
    XChangeProperty(dpy, root, netatoms[NET_SUPPORTED], XA_ATOM, 32,
